...
mydb = dbConnect(MySQL(), user='root', password='Macrobio21!', dbname='mass_spectra', host = '127.0.0.1')  #or host = 'local'
...
#results = matrix(data = NA, nrow =  dim(val1)[1]+dim(val2)[1]+dim(val3)[1] , ncol =  2000 + 4) for three layer and 2000 epoch value
results = matrix(data = NA, nrow =  dim(val1)[1]+dim(val2)[1], ncol =  1000 + 4)
row = 0
...
for (lay in 1:3){
  for (repet in 1:dim(value[[lay]])[1]){
    ...
    fc2 <- mx.symbol.FullyConnected(drop1, name="fc2", num_hidden=l2)
